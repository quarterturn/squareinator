...
// even if all notes have cleared the stack
byte lastPitch = 0;
...
void setup() {
  ...
  // serial setup (for debugging via USB)
  Serial.begin(9600);
...
void doNoteOn(byte channel, byte pitch, byte velocity)
{
  ...
  {
    Serial.print("Note: ");
    Serial.println(notes[0].myPitch);
  }

  // handle different glide settings 
  switch (glideMode)
  {
    // no glide
    case 0:
  ...
  // glide if enabled and glide time not zero  
...
void doNoteOff(byte channel, byte pitch, byte velocity)
{
  ...
  // save the value of the last note for use in gliding stacatto notes
  // lastNote = notes[0].myPitch
...
void doCC(byte channel, byte ccnumber, byte ccvalue)
{
  switch (ccnumber)
  {
    ...
    case 89:
      ...
      else if ((glideValue > 33) && (glideValue < 91))
      {
        glideMode = 1;
      }
      else if (glideValue > 90)
      {
        glideMode = 2;
      }
      break;
  ...
  switch (vcfEnvState)
  {
    ...
    case STATE_A:
      ...
      if (vcfAValue == 0)
      {
        ...
        {
          Serial.print("DAC: ");
          Serial.println(vcfCutoffOutput);
        }
